...
public:
    int canCompleteCircuit(vector<int>& gas, vector<int>& cost) {
        ...
        for (uint32_t i = 0; i < sz; ++i) {
            ...
            // If the gas in tank went negative,
            // the previous sequence can't contribute to final solution
            if (tank < 0) {
                start = i + 1;
                tank = 0;
            }
        ...
        // If we left the loop by reaching i == sz,
        // no need to check [i, i + start],
        // because the solution is guaranteed to exist
        
        // We can simply return found solution
        return start;
    }
};
// @lc code=end
